...
pub struct SmallOrb {
    ...
    bullet_speed: f32, // Speed at which spawned bullets travel
    ...
    fn tick(
        ...
        if now - self.last_attack > self.attack_timeout_ms {
            for i in 0..3 {
                ...
                let mut angle = (-pos.y + player_pos.y).atan2(-pos.x + player_pos.x);
                if i == 0 {
                    angle -= 15.0 * PI / 180.0;
                } else if i == 2 {
                    angle += 15.0 * PI / 180.0;
